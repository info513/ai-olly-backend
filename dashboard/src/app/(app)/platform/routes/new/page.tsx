...
import { useCreateRoute, ROUTE_TYPES, isValidRouteKey, type RouteType } from "@/data/platform-routes";
...
export default function NewRoutePage() {
  ...
  const [routeType, setRouteType] = React.useState<RouteType>("walking");
  const [shortDescription, setShortDescription] = React.useState("");
  const [err, setErr] = React.useState<string | null>(null);

  const effectiveKey = keyTouched ? key : slugify(name);
  const canSubmit = !!currentDestination && name.trim().length >= 2 && isValidRouteKey(effectiveKey);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const id = await create.mutateAsync({
        ...
        key: effectiveKey, name: name.trim(), route_type: routeType,
        short_description: shortDescription.trim() || null,
      });
      router.push(`/platform/routes/${id}`);
    } catch (e: any) {
      setErr(e?.code === "23505" ? `Key “${effectiveKey}” already exists in this destination.` : (e?.message ?? "Failed to create route."));
    }
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 p-4 sm:p-6">
      ...
      <header>
        <h1 className="text-xl font-semibold text-ink-primary">New route</h1>
        <p className="mt-1 text-sm text-ink-tertiary">
          Creates a <span className="font-medium text-ink-secondary">draft</span> in{" "}
          <span className="font-medium text-ink-secondary">{currentDestination?.name ?? "…"}</span>. You’ll add stops, details, and publish next.
        </p>
      </header>
      ...
      {!currentDestination ? (
        ...
        <form onSubmit={submit} className="space-y-4 rounded-xl border border-border-subtle bg-surface-raised p-5">
          <div className="space-y-1">
            ...
            <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Old Town Highlights Walk" autoFocus />
          </div>
          <div className="space-y-1">
            <label className={LABEL} htmlFor="key">Key</label>
            <Input id="key" value={effectiveKey} onChange={(e) => { setKeyTouched(true); setKey(slugify(e.target.value)); }} placeholder="old-town-highlights-walk" />
            <p className="text-[11px] text-ink-tertiary">Lowercase, hyphenated. Unique within this destination.</p>
          </div>
          <div className="space-y-1">
            <label className={LABEL} htmlFor="type">Route type</label>
            <select id="type" className={SELECT_CLS} value={routeType} onChange={(e) => setRouteType(e.target.value as RouteType)}>
          ...
          <div className="flex items-center justify-end gap-2 border-t border-border-subtle pt-4">
            <Button asChild variant="ghost" size="sm"><Link href="/platform/routes">Cancel</Link></Button>
