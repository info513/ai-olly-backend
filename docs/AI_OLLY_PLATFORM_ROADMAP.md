...
- **Package C (Steps 11–13)** applied & verified: **Storage & Asset Manager** (3 buckets, public path-validated + private signed-URL-only, assets/usages, size limits, transform/video strategy), **Newsletter** (consent-linked subscribers, validated-rule segments, versioned templates, immutable campaign snapshots, append-only events, idempotent webhooks — Brevo-ready, no send), **Analytics Foundation** (4 tenant-safe daily aggregates, idempotent tz-aware refresh, versioned formulas, role-scoped, no PII). Verification **115/115**.
- **The full Supabase data layer (Steps 1–13) is complete.** All regressions green (Step 1–4 + Package A + Package B).
- **Not started (final package):** Dashboard implementation, provider cutover, Airtable export/import + compare mode, production Supabase project, `DATA_PROVIDER` switch.
