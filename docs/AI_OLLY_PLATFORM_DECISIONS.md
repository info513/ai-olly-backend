...
>
> **Implementation status (2026-08-01):** **Step 1** (cross-cutting) and **Step 2** (tenancy & identity: destinations, hotel_groups, hotels, profiles, hotel_memberships + tenant RLS) are **applied to `aiolly-dev`**. Step 1 fail-closed/append-only (35/35 incl. anon); Step 2 tenant-isolation RLS (50/50). Docs: `DATABASE_MIGRATION_STEP_1.md`, `DATABASE_MIGRATION_STEP_2.md`. Finding (now structurally fixed): Supabase default privileges over-grant → `REVOKE ALL`/`GRANT minimum` + `alter default privileges` for future public tables. Rooms/later domains not started; production/Airtable/PWA untouched.
