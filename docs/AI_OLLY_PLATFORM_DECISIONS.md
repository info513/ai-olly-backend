...
>
> **Implementation status (2026-08-02):** **Steps 1–7** are **applied to `aiolly-dev`**: cross-cutting; tenancy & identity; rooms & Room Guide (Pattern C); Hotel Services (Pattern A, publishing + critical ack); **Destination Content (canonical, platform-owned), Presentation Layer (Pattern B: `hotel_{poi,route,whisper,event}_settings` + `resolved_destination_*`), Pricing (Pattern A: `price_categories`/`price_items`, currency/VAT/billing-unit/validity/override, `resolved_price_items` with net/gross, `pms_metadata` placeholder — no PMS)**. All wired into Step 1 versioning/audit/retention (no duplicate systems). Verification: Step 1 35, Step 2 50, Step 3 40, Step 4 76, **Package A (5/6/7) 90**. Docs: `DATABASE_MIGRATION_STEP_1.md`…`_STEP_4.md`, `_STEP_5_6_7.md`. Findings (fixed forward): default-privilege over-grant; last-admin cascade; `USAGE on schema platform`; `platform` functions not PostgREST-reachable → RPC in `public`; canonical publish across 4 tables via one whitelisted-`%I` SECURITY DEFINER fn. **Package B (Steps 8–10)** applied to `aiolly-dev`: AI Knowledge CMS (structured articles, platform/destination/hotel + override resolution, aliases, ai_configs, response logs, unanswered questions, embeddings placeholder), Guests/Stays/Consent (no accounts, duplicate-suggestions, cross-hotel guards, immutable signed consents + additive revocation, pseudonymization), Reception (requests, append-only events, feedback, push with hidden secrets). All on Step 1 versioning/audit/retention; redacted audit; sensitive columns hidden. **Package B 136/136**; regressions 35/50/40/76/90. Docs: `DATABASE_MIGRATION_PACKAGE_B.md`. Findings (forward-fixed): audit_log FK-decoupled from hotels (forensic log outlives entities so cascade deletes work); enum cast in the request-history trigger. **Package C (Steps 11–13)** applied to `aiolly-dev`: Storage & Asset Manager (3 buckets, public path-validated + private signed-URL-only, assets/usages, size limits, soft-delete guards), Newsletter (consent-linked subscribers, validated-rule segments, versioned templates, immutable campaign snapshots, append-only events, idempotent webhooks, consent-filtered audience — Brevo-ready, no send), Analytics Foundation (4 tenant-safe daily aggregates, idempotent tz-aware refresh, versioned `v1` formulas, role-scoped, no PII). **Package C 115/115**; regressions 35/50/40/76/90/136. Docs: `DATABASE_MIGRATION_PACKAGE_C.md`. Discoveries: `postgres` manages Storage buckets/object-policies in migrations; private buckets need no RLS policies (service-role signed URLs); pass `date` params as text (pg-node Date TZ off-by-one). Final package (dashboard / provider cutover / Airtable migration / compare mode / production Supabase) not started; production/Airtable/PWA untouched.
