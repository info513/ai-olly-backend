...
>
> **Implementation status (2026-08-01):** **Steps 1–4** are **applied to `aiolly-dev`**: cross-cutting; tenancy & identity; rooms & Room Guide (Pattern C); **Hotel Services & operational content (Pattern A: `service_categories`, `hotel_services`, `hotel_service_settings`, structured JSONB body, Draft→Preview→Published→Live publishing with immutable versions + critical acknowledgement, `resolved_hotel_services`)**. Verification: Step 1 35/35, Step 2 50/50, Step 3 40/40, **Step 4 76/76**. Docs: `DATABASE_MIGRATION_STEP_1.md`…`_STEP_4.md`. Findings (fixed forward): default-privilege over-grant → `REVOKE ALL`/`GRANT minimum`; last-admin guard blocked hotel cascade-delete → owner/superuser exempt; SECURITY INVOKER triggers need `USAGE on schema platform`; `platform`-schema functions aren't PostgREST-reachable → publish/rollback in `public`. POIs/destination content/routes/pricing/AI-knowledge/guests/reception/newsletter/media not started; production/Airtable/PWA untouched.
