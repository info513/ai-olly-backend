...
>
> **Implementation status (2026-08-01):** **Step 1** (cross-cutting), **Step 2** (tenancy & identity), and **Step 3** (rooms & Room Guide: `room_types`, `rooms`, `resolved_rooms` + Pattern C inheritance) are **applied to `aiolly-dev`**. Step 1 fail-closed/append-only (35/35); Step 2 tenant-isolation RLS (50/50); Step 3 inheritance + column-guards + token-hiding (40/40). Docs: `DATABASE_MIGRATION_STEP_1.md`, `_STEP_2.md`, `_STEP_3.md`. Findings (fixed forward): default-privilege over-grant → `REVOKE ALL`/`GRANT minimum`; last-admin guard blocked hotel cascade-delete → owner/superuser exempt; SECURITY INVOKER triggers need `USAGE on schema platform`. Services/later domains not started; production/Airtable/PWA untouched.
