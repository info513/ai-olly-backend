...
- **Migration Step 4 (Hotel Services & operational content: `service_categories`, `hotel_services`, `hotel_service_settings` + structured JSONB body + Pattern A default→override resolution + publishing/versioning + audit) — APPLIED to `aiolly-dev`** (2026-08-01). Draft→Preview→Published→Live workflow; publish via SECURITY DEFINER `public.publish_hotel_service()` writing immutable `content_versions`; critical-content acknowledgement (editors cannot bypass); explicit independent visibility (`visible_in_pwa`/`visible_in_web`/`available_to_ai`); `public.resolved_hotel_services(hotel)` deterministic live set (override wins, no duplicates, validity/visibility honored); redacted audit triggers. **Step 4 verification 76/76; Step 1/2/3 regressions 35/50/40.** See `docs/DATABASE_MIGRATION_STEP_4.md`. Finding: `platform`-schema functions aren't PostgREST-reachable → publish/rollback moved to `public` (forward-fix migration).
- **Package A — Steps 5+6+7 (Destination Content + Presentation Layer + Pricing) — APPLIED to `aiolly-dev`** (2026-08-02). Step 5: platform-owned canonical `destination_pois`/`destination_routes`/`destination_whispers`/`destination_events` (publish via `public.publish_destination_content`, immutable versions, platform-admin-only). Step 6 (**Pattern B**): `hotel_{poi,route,whisper,event}_settings` presentation overlay (no field-level merge) + `resolved_destination_*(hotel)`. Step 7 (**Pattern A**): `price_categories`/`price_items` (currency/VAT/billing-unit/validity, hotel override, `pms_metadata` placeholder — no PMS), `public.publish_price_item`, `resolved_price_items(hotel)` with computed net/gross. All wired into Step 1 `content_versions`/`audit_log`/`retention_policies` (no duplicate version systems). **Package A verification 90/90; Step 1/2/3/4 regressions 35/50/40/76.** See `docs/DATABASE_MIGRATION_STEP_5_6_7.md`.
- **Package B — Steps 8+9+10 (AI Knowledge + Guests/Stays/Consent + Reception) — APPLIED to `aiolly-dev`** (2026-08-02). Step 8: `knowledge_categories/articles/article_sources/aliases`, `ai_configs`, `ai_response_logs`, `unanswered_questions`, `knowledge_embeddings` (placeholder); `resolved_ai_knowledge(hotel,locale,preview)` (hotel→destination→platform, deduped, preview via RLS), `resolved_ai_config`; publish/rollback + critical ack. Step 9: `guests` (PII-restricted, pseudonymize fn), `guest_duplicate_suggestions` (no auto-merge), `stays` (cross-hotel guards, hashed access ref, `resolved_active_stay`/`resolved_stays`), `consent_templates` + immutable `consents` (`sign_consent`/`revoke_consent`, additive revocation). Step 10: `guest_requests`, append-only `request_events`, `feedback`, `push_subscriptions` (secret cols hidden), safe `guest_request_public` view. All wired into Step 1 versioning/audit/retention; redacted audit (no PII/tokens/secrets). **Package B verification 136/136; Step 1/2/3/4 + Package A regressions 35/50/40/76/90.** Findings (forward-fixed): audit_log FK-decoupled from hotels (forensic log outlives entities); enum cast in history trigger. See `docs/DATABASE_MIGRATION_PACKAGE_B.md`.
- **Package C — Steps 11+12+13 (Storage & Asset Manager + Newsletter + Analytics Foundation) — APPLIED to `aiolly-dev`** (2026-08-02). Step 11: `assets`/`asset_usages` + 3 Storage buckets (`public-media` public; `private-documents`/`consent-files` backend-signed-URL-only), per-type size limits, path-validated public writes (`can_manage_media`), usage-scope guard, soft-delete-blocked-while-used, `finalize_asset`/`asset_usage_report`. Step 12: `newsletter_subscribers` (consent-linked), `newsletter_segments`(+members, validated rule JSONB — no arbitrary SQL), versioned `newsletter_templates`, `newsletter_campaigns` (immutable snapshot after `schedule_campaign`), append-only `newsletter_events`, idempotent `newsletter_webhook_events`, `resolve_newsletter_audience` (consent-filtered) — Brevo-ready, **no send**. Step 13: `ai_quality_daily`/`operations_daily`/`newsletter_daily`/`content_health_daily` + idempotent tz-aware `refresh_*` (SECURITY DEFINER, internal authz), versioned formulas (`calc_version=v1`), role-scoped RLS, no PII. All on Step 1 versioning/audit/retention; redacted audit. **Package C verification 115/115; Step 1–4 + A + B regressions 35/50/40/76/90/136.** See `docs/DATABASE_MIGRATION_PACKAGE_C.md`.
- **Final package (dashboard, provider cutover, Airtable export/import, compare mode, production Supabase) not started.** Production, Airtable, Render and the guest PWA **untouched**; `DATA_PROVIDER=airtable`.
